...
def sigma(R, state, action, V):             ########### sigma function defined in experiments section. 
    ...
    for prob in corners:
        E = 0
        for state in set(succ):
            E = E + prob[state] * V[state]
        E_corners.append(E)
    return min(E_corners)
...
def get_right(state):
    ...
    state_right = state if state_right in holes else state_right
    return state_right
def get_left(state):
    i = int(state / grid_size)
    j = int(state % grid_size)
    state_left = state if j == 0 else state - 1
    state_left = state if state_left in holes else state_left
    return state_left
def get_up(state):
    i = int(state / grid_size)
    j = int(state % grid_size)
    state_up = state if i == 0 else state - grid_size
    state_up = state if state_up in holes else state_up
    return state_up
def get_down(state):
    i = int(state / grid_size)
    j = int(state % grid_size)
    state_down = state if i == grid_size - 1 else state + grid_size
    state_down = state if state_down in holes else state_down
    return state_down
def make_MDP(n):
...
if __name__ == '__main__':
    ...
    for n in range(2,11):
        ...
        V=run_RVI(RPPI_value, 10**-3)
        print("RVI Value: "+str(V[0]))
        RVI_time = round(time.time()*1000) - RVI_time
        RVI_time_plot.append(RVI_time)
        print(f"RVI time: {RVI_time}")

    # plt.plot(RVI_time_plot, label='Robust Value Iteration')
    ...
    file = open('RVI_time_lake.txt','w')
    file.write(str(RVI_time_plot))
    file.close()

    file = open('RPPI_time_lake.txt','w')
